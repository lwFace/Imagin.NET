...
namespace Imagin.Common.Serialization
{
    public class BinarySerializer
    {
        public static Result Deserialize<T>(string filePath, out T data)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"{nameof(BinarySerializer)}.{nameof(Deserialize)}: {e.Message}");
                result = new Error(e);
            }
            finally
            {
        ...
        public static Result Serialize(string filePath, object data)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"{nameof(BinarySerializer)}.{nameof(Serialize)}: {e.Message}");
                result = new Error(e);
            }
            finally
            {
