    ...
    public sealed class Folder : Container
    {
        ...
        public class Long : BaseLong
        {
            ...
            static void InternalGetDirectories(string path, string searchPattern, System.IO.SearchOption searchOption, ref List<string> dirs)
            {
                ...
                var findHandle = default(IntPtr);

                try
                {
                    findHandle = FindFirstFile(System.IO.Path.Combine(GetWin32LongPath(path), searchPattern), out findData);
            ...
            public static string ClonePath(string folderPath, string nameFormat = Storage.Path.DefaultCloneFormat) => Storage.Path.Clone(folderPath, nameFormat, i => Exists(i));
            ...
            {
                //Not sure if a period (repeated any number of times) would need to be considered a valid folder path 
                //in the context of most operations...
                if (path.OnlyContains('.'))
                    return false;
            ...
            async public static Task<long> GetSize(string folderPath, System.Threading.CancellationToken token)
            {
                long result = 0;
            ...
            public static IEnumerable<string> GetFiles(string path, SearchOption searchOption = SearchOption.TopDirectoryOnly)
            {
                ...
                if (path == RecycleBin)
                    path = @"C:\$RECYCLE.BIN\S-1-5-21-2499919508-2746791852-2305744323-1001";
                ...
                foreach (var i in directories)
                {
                    ...
                    try
                    {
                        findHandle = FindFirstFile(System.IO.Path.Combine(GetWin32LongPath(i), searchPattern), out findData);
                        if (findHandle != new IntPtr(-1))
                        {
                            do
                            {
                ...
                foreach (var i in files)
                {
                    ...
                    {
                        //This particular file is used by Windows to determine how a folder is displayed (in Windows Explorer). 
                        //As such, ignore it!
                        continue;
                    }
                    yield return i;
            ...
            public static string Parent(string folderPath)
            {
                ...
                return result.Parent == null ? Root : result.Parent.FullName;
            }
