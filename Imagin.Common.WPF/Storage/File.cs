...
namespace Imagin.Common.Storage
{
    public class File : Item
    {
        ...
        public class Long : BaseLong
        {
            ...
            public static string CleanName(string fileName) => System.IO.Path.GetInvalidFileNameChars().Aggregate(fileName, (current, c) => current.Replace(c.ToString(), string.Empty));
            ...
            public static string ClonePath(string path, string nameFormat = Storage.Path.DefaultCloneFormat) => Storage.Path.Clone(path, nameFormat, i => Exists(i));
            ...
            public static string Relative(string path, Index index, System.Random random = null)
            {
                if (Storage.File.Long.Exists(path))
                {
                    var files = Directory.EnumerateFiles(System.IO.Path.GetDirectoryName(path)).ToList<string>();
                    if (files.Any<string>())
                    {
                        ...
                        switch (index)
                        {
                            case File.Index.Next:
                                i++;
                                break;
                            case File.Index.Previous:
                                i--;
                                break;
                            case File.Index.First:
                                i = 0;
                                break;
                            case File.Index.Last:
                                i = c - 1;
                                break;
                            case File.Index.Random:
            ...
            public static ImageSource Thumbnail(string path)
            {
                ...
                double angle = 0;
                ExifOrientations orientation = ExifOrientations.None;
